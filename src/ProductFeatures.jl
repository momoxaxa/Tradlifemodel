...
function premium(input_tables_dict::Dict, mp::ModelPoint, pol_year, modal_cf_indicator, duration, product_features_set::ProductFeatureSet)
    ...
    if product_features_set.premium.table_type == "User Defined Table"
        ...
        for var in variables
            val = setproperty!(ProductFeatures, var, read_table_PY(input_tables_dict[product_features_set.premium.table], string(var), pol_year, duration))
            @eval $var = $val
        end
    ...
    elseif product_features_set.premium.table_type == "Rate per 1000 SA by Issue Age/Pol Term/Prem Term"
        assumptions_array = read_table_EA_MI(input_tables_dict[product_features_set.premium.table], mp.pol_term, mp.prem_term, "Value", mp.issue_age) .* mp.sum_assured ./ 1000
    end
...
function death_benefit(input_tables_dict::Dict, mp::ModelPoint, pol_year, duration, product_features_set::ProductFeatureSet)
    ...
    if product_features_set.death_ben.table_type == "User Defined Table"
        ...
        for var in variables
            val = read_table_PY(input_tables_dict[product_features_set.death_ben.table], string(var), pol_year, duration)
            @eval $var = $val
        end
        assumptions_array = eval(formula)
...
function surr_benefit(input_tables_dict::Dict, mp::ModelPoint, pol_year, duration, product_features_set::ProductFeatureSet)
    ...
    if product_features_set.surr_ben.table_type == "User Defined Table"
        ...
        for var in variables
            val = setproperty!(ProductFeatures, var, read_table_PY(input_tables_dict[product_features_set.surr_ben.table], string(var), pol_year, duration))
            @eval $var = $val
        end
        assumptions_array = eval(formula)
    ...
    elseif product_features_set.surr_ben.table_type == "Rate per 1000 SA by Pol Year/Issue Age/Prem Term"
        assumptions_array = read_table_PY_MI(input_tables_dict[product_features_set.surr_ben.table], mp.issue_age, mp.prem_term, "Value", pol_year) .* mp.sum_assured ./ 1000
    end
    return assumptions_array .* mult
end
...
function comm_perc(input_tables_dict::Dict, mp::ModelPoint, pol_year, duration, product_features_set::ProductFeatureSet)
    ...
    if product_features_set.commission.table_type == "User Defined Table"
        ...
        for var in variables
            val = setproperty!(ProductFeatures, var, read_table_PY(input_tables_dict[product_features_set.commission.table], string(var), pol_year, duration))
            @eval $var = $val
        end
        assumptions_array = eval(formula)
    ...
    elseif product_features_set.commission.table_type == "Perc by Pol Year/Pol Term/Prem Term"
        assumptions_array = read_table_PY_MI(input_tables_dict[product_features_set.commission.table], mp.pol_term, mp.prem_term, "Value", pol_year)
    end
    return assumptions_array .* mult
end